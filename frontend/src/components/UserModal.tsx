          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p style={{ fontSize: 10, fontWeight: 600, color: '#8b949e', textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: 2 }}>
                {isEdit ? form.username : 'Completa los datos del usuario'}
              </p>
            </div>
          </div>
        ...
        <form id="um-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-7 space-y-6">
          <section>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              <div>
                <FL>Nombre *</FL>
                <FI name="nombre" value={form.nombre} onChange={handleChange} required />
              </div>
              <div>
                <FL>Apellidos *</FL>
                <FI name="apellidos" value={form.apellidos} onChange={handleChange} required />
              </div>
              <div className="md:col-span-2">
                <FL>Email *</FL>
                <FI name="email" type="email" value={form.email} onChange={handleChange} required />
              </div>
              <div>
                <FL>Teléfono</FL>
                <FI name="telefono" value={form.telefono ?? ''} onChange={handlePhoneChange} />
              </div>
              <div>
                <FL>Horario</FL>
                <FI name="horario" value={form.horario ?? ''} onChange={handleChange} />
              </div>
              <div>
                <FL>Rol *</FL>
