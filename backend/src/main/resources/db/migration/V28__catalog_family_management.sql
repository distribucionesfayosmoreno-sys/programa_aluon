...
BEGIN
    ...
    WHERE t.relname = 'aluon_saas_catalogo_productos'
      AND c.contype = 'u'
      AND pg_get_constraintdef(c.oid) LIKE '%tenant_id, modelo%';

    IF legacy_constraint_name IS NOT NULL THEN
        EXECUTE format('ALTER TABLE aluon_saas_catalogo_productos DROP CONSTRAINT %I', legacy_constraint_name);
    END IF;
END $$;
