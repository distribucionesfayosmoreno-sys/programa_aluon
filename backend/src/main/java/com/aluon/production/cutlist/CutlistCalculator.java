...
public class CutlistCalculator {
    ...
    private List<CutlistLine> generateAbatibleDos(CutlistRequestDto request) {
        ...
        addLamasDoble(items, modelo, numeroDeLamasEntero, lamaRestante, (medidaAnchoInterior / 2.0) - holguraLamasHorizontal);
    ...
    private List<CutlistLine> generateCorredera(CutlistRequestDto request) {
        ...
        int marcoInferior = cola ? ancho + 250 : ancho;
        ...
        if (modelo == DoorModel.PREMIUM) {
            ...
            if (lamaRestante > 0) {
                addItem(items, "Lama adicional Lama 200x20 corte recto", 1, lamaRestante + "mm X " + medidaAnchoInterior);
            }
        } else if (modelo == DoorModel.CLASSIC) {
            addItem(items, "Lama 100x20 corte recto", numeroDeLamasEntero, medidaAnchoInterior);
            if (lamaRestante > 0) {
                addItem(items, "Lama adicional Lama 100x20 corte recto", 1, lamaRestante + "mm X " + medidaAnchoInterior);
            }
        } else if (modelo == DoorModel.INOX) {
            ...
            if (lamaRestante > 0) {
                addItem(items, "Lama adicional Lama 100 Avión corte recto", 1, lamaRestante + "mm X " + medidaAnchoInterior);
            }
        }
    ...
    private void addMarcoInterior(List<CutlistLine> items, DoorModel modelo, int medidaAnchoInterior, int medidaAlturaInterior) {
        if (modelo == DoorModel.INOX) {
    ...
    private void addMarcoInteriorVeneciana(List<CutlistLine> items, String horizontalDesc, String verticalDesc, double medidaAnchoInterior, double medidaAlturaInterior) {
        addItem(items, horizontalDesc, 2, medidaAnchoInterior);
        addItem(items, verticalDesc, 2, medidaAlturaInterior);
    }

    private void addMarcoInteriorDoble(List<CutlistLine> items, DoorModel modelo, int medidaAnchoInterior, int medidaAlturaInterior) {
        if (modelo == DoorModel.VENECIANA) {
            addItem(items, "Marco Horizontal 50x50 troquelado con corte inglete 45º Contrario", 2, medidaAnchoInterior / 2.0);
            ...
            addItem(items, "Marco Vertical 80x50 y pestaña con corte inglete 45º Contrario", 1, medidaAlturaInterior + 40);
        } else {
            addItem(items, "Marco Horizontal 80x50 con ranura con corte inglete 45º Contrario", 4, medidaAnchoInterior / 2.0);
            addItem(items, "Marco Vertical 80x50 con ranura con corte inglete 45º Contrario", 3, medidaAlturaInterior);
            addItem(items, "Marco Vertical 80x50 con ranura y pestaña con corte inglete 45º Contrario", 1, medidaAlturaInterior + 40);
        }
    }

    private void addLamas(List<CutlistLine> items, DoorModel modelo, int numeroDeLamasEntero, int lamaRestante, double medidaAncho) {
        if (modelo == DoorModel.PREMIUM) {
            addItem(items, "Lama 200x20 corte recto", numeroDeLamasEntero, medidaAncho);
            if (lamaRestante > 0) {
                addItem(items, "Lama adicional Lama 200x20 corte recto", 1, lamaRestante + "mm X " + formatNumber(medidaAncho));
            }
        } else if (modelo == DoorModel.CLASSIC) {
            addItem(items, "Lama 100x20 corte recto", numeroDeLamasEntero, medidaAncho);
            if (lamaRestante > 0) {
                addItem(items, "Lama adicional Lama 100x20 corte recto", 1, lamaRestante + "mm X " + formatNumber(medidaAncho));
            }
        } else if (modelo == DoorModel.INOX) {
            double medidaAnchoInox = medidaAncho - 35;
            addItem(items, "Lama 200x26 corte recto", numeroDeLamasEntero, medidaAnchoInox);
            if (lamaRestante > 0) {
                addItem(items, "Lama adicional Lama 200x26 corte recto", 1, lamaRestante + "mm X " + formatNumber(medidaAnchoInox));
            }
            ...
            if (lamaRestante > 0) {
                addItem(items, "Lama adicional Lama 100 Avión corte recto", 1, lamaRestante + "mm X " + formatNumber(medidaAncho));
            }
        }
    }

    private void addLamasDoble(List<CutlistLine> items, DoorModel modelo, int numeroDeLamasEntero, int lamaRestante, double medidaAncho) {
        int totalLamas = numeroDeLamasEntero * 2;
        if (modelo == DoorModel.PREMIUM) {
            addItem(items, "Lama 200x20 corte recto", totalLamas, medidaAncho);
            if (lamaRestante > 0) {
                addItem(items, "Lama adicional Lama 200x20 corte recto", 2, lamaRestante + "mm X " + formatNumber(medidaAncho));
            }
        } else if (modelo == DoorModel.CLASSIC) {
            addItem(items, "Lama 100x20 corte recto", totalLamas, medidaAncho);
            if (lamaRestante > 0) {
                addItem(items, "Lama adicional Lama 100x20 corte recto", 2, lamaRestante + "mm X " + formatNumber(medidaAncho));
            }
        } else if (modelo == DoorModel.INOX) {
            double medidaAnchoInox = medidaAncho - 35;
            addItem(items, "Lama 200x26 corte recto", totalLamas, medidaAnchoInox);
            if (lamaRestante > 0) {
                addItem(items, "Lama adicional Lama 200x26 corte recto", 2, lamaRestante + "mm X " + formatNumber(medidaAnchoInox));
            }
            ...
            if (lamaRestante > 0) {
                addItem(items, "Lama adicional Lama 100 Avión corte recto", 2, lamaRestante + "mm X " + formatNumber(medidaAncho));
            }
        }
    }
