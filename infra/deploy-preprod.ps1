Param(
  ...
  [string]$DbName = "aluonbbddpreprod",
  [string]$DbUser = "aluon",
...
$remoteBootstrap = @"
set -e
if [ ! -d "$AppDir-preprod/.git" ]; then
  mkdir -p "$AppDir-preprod"
  git clone "$RepoUrl" "$AppDir-preprod"
fi
cd "$AppDir-preprod"
...
if [ -z "$ComposePath" ]; then
  if [ -f "infra/docker-compose.$EnvName.yml" ]; then
    ...
    echo "Compose file not found in $AppDir-preprod"
    exit 1
  fi
else
...
ENV_FILE=""
if [ -f "infra/.env.pre" ]; then
  ENV_FILE="--env-file infra/.env.pre"
fi
...
Write-Host "Done. PREPROD deployed at http://${RemoteHost}:${FrontendPort}"
